...
function addTitle() {
  ...
  const title = $("<div class='title'>Ronaldo vs Messi</div>").css({
    top: "35px", // Von 20px auf 35px verschoben
    left: "30px",
    color: "black",
  });
...
function drawEntries(player, startX, initialY, entriesPerColumn, gap) {
  ...
  types.forEach((type) => {
    if (type === "Header") {
      const entries = RonaldoMessiDataGroup[player].filter(
  ...
  const leftFootedEntries = RonaldoMessiDataGroup[player].filter(
    (entry) => entry.Type === "Left-footed shot"
  );
  const leftFootedDivs = createRow(
    leftFootedEntries,
    shotY,
    currentX,
    true,
  ...
  const leftFootedColumnWidth = calculateColumnWidth(
    leftFootedEntries,
    entriesPerColumn
  );
  currentX += leftFootedColumnWidth + gap;

  const rightFootedEntries = RonaldoMessiDataGroup[player].filter(
    (entry) => entry.Type === "Right-footed shot"
  );
  const rightFootedDivs = createRow(
    rightFootedEntries,
    shotY,
    currentX,
    true,
...
function createRow(entries, startY, startX, isVertical, entriesPerColumn) {
  ...
  entries.forEach((entry, index) => {
    ...
    let opacity = getCircleOpacity(entry);
    ...
    circle.attr(
      "check",
      String(entry.Minute).includes("45+") ||
        String(entry.Minute).includes("90+")
    );
...
function getCircleSize(matchday) {
  switch (matchday) {
    case "Final":
      return "19px"; // Zurück zur ursprünglichen Größe
    case "Semi-Finals":
      return "15px"; // Zurück zur ursprünglichen Größe
    case "Quarter-Finals":
      return "12px"; // Zurück zur ursprünglichen Größe
    case "Group Stage":
      return "9px"; // Zurück zur ursprünglichen Größe
    default:
      return "9px"; // Zurück zur ursprünglichen Größe
  }
}
...
function getCircleColor(entry) {
  ...
  if (isWin) {
    return "#00CC6D"; // Siege: Ursprüngliches Grün
  } else if (isLoss) {
    return "#BF211E"; // Niederlagen: Ursprüngliches Rot
  } else {
    return "#8846D2"; // Unentschieden: Ursprüngliches Lila
  }
}

function getCircleOpacity(entry) {
  if (!entry || !entry.Result) {
    return "1";
  }
  ...
  return "0.9"; // Alle haben 90% opacity für optimalen sanften Look
}
...
function resetAllGoals() {
  renderer.find(".goal").each(function () {
    ...
    if (entry) {
      ...
      const opacity = getCircleOpacity(entry);
...
function showEmptyView() {
  ...
  lastYCR7 = drawFilteredEntries(
    CR7,
    ronaldoContainerStartX,
    lastYCR7,
    "neither"
  );
  lastYLM10 = drawFilteredEntries(
    LM10,
    messiContainerStartX,
    lastYLM10,
    "neither"
  );
...
function drawMatchdayEntries(player, startX, startY) {
  ...
  const remainingEntries = RonaldoMessiDataGroup[player].filter(
    (entry) => !matchdayOrder.includes(entry.Matchday)
  );
...
function applyHalfFilter() {
  ...
  renderer.find(".goal").each(function () {
    ...
    if (isShowingFirstHalf) {
      ...
      let opacity = getCircleOpacity(entry);
      const backgroundImage =
        Number(minute) < 46 || String(minute).includes("45+")
          ? `linear-gradient(to right, white 50%, ${color} 50%)`
          : "none";
      circle.css({
        "background-color": color,
